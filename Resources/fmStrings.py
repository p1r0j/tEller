# Written by piroj,
# for the offline CLI budgeting tool,
# tEller.

# Faces.
fEMPTY = " |   |"
fHEAD = "[[bold cyan]~>>>~[/bold cyan]]"
fPROMPT = "([bold magenta]~[ ]~[/bold magenta])"
fERROR = "[[bold red]ERROR[/bold red]]"
fOK = " |[green]OK![/green]|"
fNOK = " |[red]~!~[/red]|"
fPLUS = " |[green]  +[/green]|"
fMINUS = " |[red]  -[/red]|"
fEQUAL = " |[yellow]  =[/yellow]|"
fRECORD = " |[cyan]>->[/cyan]|"
fSEL1 = " |[magenta]  1[/magenta]|"
fSEL2 = " |[magenta]  2[/magenta]|"
fSEL3 = " |[magenta]  3[/magenta]|"
fESC = " |[red]ESC[/red]|"
# Formatted words.
wTELLER = "[bold cyan]tEller[/bold cyan]"
wDATE = "[bold blue]Active Date[/bold blue]"
wAUTO = "[bold blue]Auto[/bold blue]"
wMANUAL = "[bold blue]Manual[/bold blue]"
wINCOME = "[bold green]Income[/bold green]"
wINC = "[bold green]Income Sources[/bold green]"
wBUDGET = "[bold yellow]Budget Categories[/bold yellow]"
wESSENTIALS = "[bold yellow]Essentials[/bold yellow]"
wNESSENTIALS = "[bold yellow]Non-Essentials[/bold yellow]"
wSAVDEBT = "[bold yellow]Savings & Debt[/bold yellow]"
wSCAT = "[bold yellow]Budget Subcategories[/bold yellow]"
wMTRANS = "[bold red]Monthly Transactions[/bold red]"
wCTRANS = "[bold red]Categorical Transactions[/bold red]"
wSTRANS = "[bold red]Subcategorical Transactions[/bold red]"

